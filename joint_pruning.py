"""
joint_pruning.py - Learnable Joint Pruning for Sign Language Recognition

This module implements Strategy C: learnable joint pruning using Gumbel-Softmax.
Instead of manually selecting joints, the model learns which joints are important.

Installation:
    1. Save this file as joint_pruning.py in your project directory
    2. Add to lightning_asl.py: from joint_pruning import JointPruningLayer, l0_penalty
    3. Follow integration_guide.md for the 5 modification points
"""
...
class JointPruningLayer(nn.Module):
    ...
    Example:
        ...
        >>> skeleton = torch.randn(4, 16, 42, 2)  # (B, T, J, P)
    ...
    def __init__(
        ...
        """
        Args:
            num_joints: Total number of skeleton joints (e.g., 42 for 2x21-hand)
    ...
    def get_selection_probs(self) -> torch.Tensor:
        """Get sigmoid probabilities for each joint (for analysis)."""
    ...
    def get_active_joints(self, threshold: float = 0.5) -> torch.Tensor:
        ...
        Args:
            threshold: Probability threshold for being "active"
            
        Returns:
            Boolean mask of shape (num_joints,)
        """
...
def l0_penalty(pruning_layer: JointPruningLayer, weight: float = 0.001) -> torch.Tensor:
    ...
    Args:
        pruning_layer: JointPruningLayer instance
